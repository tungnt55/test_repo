...
for k=1,num_process do
  ...
  boxes = result.boxes
  -- for i=1, 30 do
  --   print(captions[i],boxes[i])
  -- end
  captions_file = io.open("captions.txt","w")
  boxes_file = io.open("boxes.txt","w")
  --for key,value in pairs(result["captions"]) do
    --txt_file:write(value,".\n")
  for i=1,15 do
    captions_file:write(result["captions"][i],".\n")
    boxes_file:write(tostring(result["boxes"][i]),"\n")
  end
  captions_file:close()
  boxes_file:close()
